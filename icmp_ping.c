...
static struct timeval tv_begin,tv_end,tv_internel,tv_recv;
...
int main(int argc,char *argv[])
{
    ...
    int pactet_index=0;
    for(pactet_index = 0;pactet_index<128;pactet_index++)
    {
         pingpacket[pactet_index].flag = 0;
    }
	...
	memcpy(dest_str,argv[1],strlen(argv[1]+1)+1);
...
static int icmp_unpack(char *buf,int len)//icmp报头的剥离
{
	...
	if((icmp->icmp_type == ICMP_ECHOREPLY) && (icmp->icmp_id == (pid & 0xffff)))//是一个echo回复报文 不是其他进程的ping的回复
	{
		printf("get in\n");
